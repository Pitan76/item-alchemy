...
import net.pitan76.mcpitanlib.api.item.v3.tool.CompatPickaxeItem;
...
public class AlchemicalPickaxe extends CompatPickaxeItem implements IRechargeableFromKlein, AlchemicalToolMode {
    ...
    public boolean postMine(PostMineEvent e) {
        ...
        Direction forward = e.getPlayer().getHorizontalFacingM();
        ...
        if (mode == 0 && charge > 0) {
            ...
            mineAdjacentBlocks(e.getPlayer(), world, originPos, Direction.UP, 1);
            mineAdjacentBlocks(e.getPlayer(), world, originPos, Direction.DOWN, 1);
            Direction left = forward.rotateYCounterclockwise();
            Direction right = forward.rotateYClockwise();
            mineAdjacentBlocks(e.getPlayer(), world, originPos, left, 1);
            mineAdjacentBlocks(e.getPlayer(), world, originPos, right, 1);
        } else {
            ...
            switch (mode) {
                ...
                case 3:
                    // Depth 3 mode
                    mineAdjacentBlocks(e.getPlayer(), world, originPos, forward, 2);
                    break;
                default:
                    break;
            }
    ...
    public void getTargetBlocks(List<BlockPos> targetBlocks, World world, BlockPos blockPos, ItemStack stack, Direction direction, int distance) {
        for (int i = 1; i <= distance; i++) {
            BlockPos targetPos = blockPos.offset(direction, i);
            BlockState targetState = world.getBlockState(targetPos);
            if (isSuitableFor(new SuitableForArgs(targetState))) {
